...
//	private boolean autoTurretOn = false;
            ...
            while (mRun) {
                ...
                if(surfaceDone) {   
                	try {
                		...
                		if(ret == -1)
                		{
                			((Remote)saved_context).setImageError();
                			return;
                		}
                		
                		bmp = drawCrosshair(bmp);
                		/*
                		if(autoTurretOn){
                			autoTurret.loadBitmap(bmp);
                			bmp = autoTurret.detectFaces();
                			count = 0;
                		} */
...
//        try {
//			autoTurret = new AutoTurret(context);
//		} catch (IOException e) {
//			e.printStackTrace();
//		}
    }
...
//	public void setAutoTurret(boolean state){
//		autoTurretOn = state;
//	}
	
	private Bitmap drawCrosshair(Bitmap target){
		IplImage image = IplImage.create(target.getWidth(), target.getHeight(), IPL_DEPTH_8U, 4);
		target.copyPixelsToBuffer(image.getByteBuffer());
		cvCircle(image, cvPoint(target.getWidth() / 2, target.getHeight() / 2), 5, CvScalar.BLUE, -1, 8, 0);
		Bitmap alteredPicture = Bitmap.createBitmap(image.width(), image.height(), Bitmap.Config.ARGB_8888);
		alteredPicture.copyPixelsFromBuffer(image.getByteBuffer());
		return alteredPicture;
	}
}
